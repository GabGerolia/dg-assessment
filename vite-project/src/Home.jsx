...
function Home() {
  ...
  useEffect(() => {
    if (user?.id) {
      axios.get(`http://localhost:8080/api/projects/${user.id}`)
        .then(res => {
          if (res.data.success) {
  ...
  const handleCreateProject = (project) => {
    if (editingProject) {
      axios.put(`http://localhost:8080/api/projects/${editingProject.id}`, {
        title: project.title,
      ...
      axios.post("http://localhost:8080/api/projects", {
        ...
        if (res.data.success) {
          setProjects(prev => [...prev, { id: res.data.id, ...project }]);
        }
      });
    }
    setShowModal(false);
  ...
  const handleDeleteConfirm = () => {
    axios.delete(`http://localhost:8080/api/projects/${projectToDelete}`)
      .then(res => {
        if (res.data.success) {
